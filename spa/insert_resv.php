...
$type_soin=$_POST['types'];echo $type_soin.'<br>';
$soin=$_POST['soin'];echo ("soin $soin").'<br>';
...
$note= clean ($_POST['note']);//echo $note.'<br>';


$client_ext=$_POST['client_externe'];//echo $client_ext.'<br>';
$paye=$_POST['paye'];//echo $paye.'<br>';
...
//$idcl = $idc+1; //echo $id;
$sql=mysql_query("INSERT INTO client (genre,name,last,email,type,phone,room,health,id_opera,date_ajout,auteur) VALUES
	                          ('Mr','$name','$last','$email','$type','$phone','$room','$health','$opera','$datenow','$moi')");
...
$sql5 ="select * from reservation WHERE ((start_time between '$commence' and '$fini') or (endtime between '$commence' and '$fini')) and (id_agent ='$agent' or  id_cabine='$cabine') and status not like 'Annuler'";
...
('$date_res','$commence','$fini','$status','$note','$paye',$cabine,'$type_soin','$soin','$agent','$leclient','$datenow','$moi')");
					...
					{echo 'ok';
					 ...
					 header('location:reservation.php?success=1&rev='.$leclient.'&f='.$fini.'');exit;
					}
