...
<?php
  ...
  <input type="text" name="date" class="form-control" id="datepicker-autoclose" value="<?php echo date('Y-m-d',strtotime($date_m)) ?>">
								...
								if(!isset ($_GET['date']))
										{ $date_cal = date('Y-m-d' );
											...
											//echo $date_cal;
										?>
										<form name="f2" method="post" action="reservation_all.php">
											 <button  type="submit" name="submit"  class="btn btn-success">
											<i class="fa fa-check">	</i> Update Selected Non-Payée</button>
											 <button  type="submit" name="submit2"  class="btn btn-danger">
											<i class="fa fa-trash-o">	</i> Delete All</button>
										<?php
										
									if(isset($_POST['submit'])){
								    if(!empty($_POST['idr'])) {										
									foreach ($_POST['idr'] as $select )
									{
									  $sql6=mysql_query("UPDATE reservation SET paye='payer' where id_res='$select' ");
								 	} 
									}
									}
												if(isset($_POST['submit2'])){
								    if(!empty($_POST['idr'])) {										
									foreach ($_POST['idr'] as $select2 )
									{
									  $sql7=mysql_query("DELETE FROM reservation  where id_res='$select2' ");
								 	} 
									}
									}
                            ...
                            <table id="example23" class="display nowrap" cellspacing="0" width="100%">
                                <thead>
								   ...
								   <?php
								  // $date_now=date("Y-m-d");
		$sql = "SELECT *  FROM reservation where date='$date_cal'    ";
 			 		$req= mysql_query ($sql) ;
...
?>
										<td>
							<input type="checkbox" name="idr[]" value="<?php echo $id ;?>"   /> 
			...
			$sql2 = "SELECT a.id_agent,a.nom ,r.id_agent FROM agent a,reservation r where r.id_agent=a.id_agent and r.id_agent=$agent  ";
				...
				$sql3 = "SELECT c.id_cabine,c.name ,r.id_cabine FROM cabine c,reservation r where r.id_cabine=c.id_cabine and r.id_cabine=$cabine  ";
									...
									<td> 
										<a href="reservation_modify.php?id=<?php echo $id ?>" data-toggle="tooltip" data-original-title="Edit"> <i class="fa fa-pencil text-inverse m-r-10"></i> </a>										
										<a href="annule_resv.php?id=<?php echo $id ?>" data-toggle="tooltip" data-original-title="Annuler"> <i class="fa fa-ban "></i> </a>
										&nbsp 
										<a href="reservation_delete.php?id=<?php echo $id ?>" data-toggle="tooltip" data-original-title="Delete"> <i class="fa fa-trash text-danger"></i> </a>
										</td>
									   <td>									  
										   ...
										   ?>
						 <div class="label label-table label-danger "><?php echo "N.P"?></div>	
					 ...
					 { ?> <div class="label label-table label-success "><?php echo "P"?></div> 
					<a  href="update_paye.php?id=<?php echo $id;?>"> <button type="button"  class="btn btn-sm btn-default disabled" data-toggle="tooltip" data-original-title=" Déja Payer"></a><i class="ti-check-box" aria-hidden="true"></i></button> 
								...
								<?php if ($status=="confirmer") {
									echo '<td><div class="label label-table label-success">'.'OK'.'</div></td>';							 }
